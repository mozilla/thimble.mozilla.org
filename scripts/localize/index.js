...
const dest = path.resolve(process.cwd(), l10nConfig.locale_dest || "dist/locales");
const locales = fs.readdirSync(src);

const JS_WRAPPER =
`window.__THIMBLE_STRINGS__= {
  get: function(key) {
    return this._values[key];
  },
  `;
const JS_WRAPPER_MIN ="window.__THIMBLE_STRINGS__={get:function(_){return this._values[_]},";

function writeFile(locale, file, properties, addWrapper) {
  locale = locale.replace(/-/g, "_");
  properties = JSON.stringify(properties, null, IS_DEVELOPMENT ? 2 : 0);
  if(addWrapper) {
    properties = `${IS_DEVELOPMENT ? JS_WRAPPER : JS_WRAPPER_MIN}_values: ${properties}};`;
  }
...
function readProperties(locale, fileName) {
  return new Promise(function(resolve, reject) {
    properties.read(path.join(src, locale, `${fileName}.properties`), function(err, properties) {
      if(err) {
        reject(err);
      } else {
        resolve(properties);
...
co(function* () {
  ...
  const enUSSharedProperties = yield readProperties("en-US", "server-client-shared");

  yield writeFile("en-US", "messages.json", Object.assign(enUSServerProperties, enUSSharedProperties));
  yield writeFile("en-US", "strings.js", Object.assign(enUSClientProperties, enUSSharedProperties), true);
  ...
  for(locale of locales.filter(l => l !== "en-US" && l !== ".DS_Store")) {
    ...
    const sharedProperties = yield readProperties(locale, "server-client-shared");

    yield writeFile(locale, "messages.json", Object.assign(enUSServerProperties, enUSSharedProperties, serverProperties, sharedProperties));
    yield writeFile(locale, "strings.js", Object.assign(enUSClientProperties, enUSSharedProperties, clientProperties, sharedProperties), true);
  }
})

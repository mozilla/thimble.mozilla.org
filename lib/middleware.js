...
module.exports = function middlewareConstructor() {
  ...
  return {
    ...
    redirectAuthenticatedUsers: function(req, res, next) {
      var qs = querystring.stringify(req.query);
      if(qs !== "") {
        qs = "?" + qs;
      }
    ...
    /**
     * Check if if a project has been loaded for the current context
     */
    isProjectLoaded: function(req, res, next) {
      if(!req.session.project || !req.session.project.meta) {
        return next(utils.error(404, "No project information available"));
      }

  ...
  return {
    ...
    checkPageOperation: function(db) {
      return function(req, res, next) {
        ...
        if (req.body.pageOperation === "remix" || !originalId) {
          return next();
        }
        ...
        db.find(originalId, function(err, result) {
          ...
          if (result.userid === req.session.email) {
            return next();
          }
    ...
    getRemixedFrom: function(db, make) {
      return function(req, res, next) {
        function findMake(url) {
          make.search({url: url}, function(err, result) {
            if (err) {
              return next(err);
            }
            if (result.length === 1) {
              req.body.remixedFrom = result[0]._id;
            }
            next();
          });
        }
        function findRemix(id) {
        ...
        function findProject(id) {
          db.find(id, function(err, result) {
            ...
            findRemix(result.originalURL);
          });
        }
        findProject(req.publishId);
      };
    },
    ...
    finalizeProject: function(nunjucksEnv, env) {
      ...
      return function(req, res, next) {
        var sanitized = req.body.sanitizedHTML,
            ...
            remixUrl = hostname + "/remix/" + req.publishId + "/remix",
            ...
            detailsRendered = detailsTemplate.render({
              ...
              CONTENT_TYPE: "application/x-thimble",
            ...
            metaTagsRender = metaTagTemplate.render({
              metaTags: generateOGData(metaData, hostname + "/remix/" + req.publishId)
            }),
          ...
          .on("response", function(res) {
            if (res.statusCode === 200) {
              ...
              ["edit","remix"].forEach(function(suffix) {
                ...
                var redirect = "<html><head><meta http-equiv='refresh' content='0; url="+thimbletarget+"'></head><body>"+suffix+" page soft-redirect</body></html>";

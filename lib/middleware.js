 ...
 */
exports.finalizeProject = function(req, res, next) {
  // FIXME: stub, needs implementation
  // See: https://bugzilla.mozilla.org/show_bug.cgi?id=861351
  // See: https://bugzilla.mozilla.org/show_bug.cgi?id=864452
  if (req.body.sanitizedHTML) {
    req.body.finalizedHTML = req.body.sanitizedHTML;
    next();
  } else { next(new Error("no sanitized data found")); }
};
  ...
  return function(req, res, next) {
    ...
    .on("response", function(res) {
      if (res.statusCode === 200) {
        // FIXME: This url will be highly dependent on what we end up
        //        with, user-domain wise.
        // See: https://bugzilla.mozilla.org/show_bug.cgi?id=863368
        var url = "http://" + s3.bucket + "." + s3.domain + ":" + s3.port + "/" + location;

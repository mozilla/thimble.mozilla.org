...
module.exports = function middlewareConstructor() {
  ...
  return {
    ...
    /**
     * Check if a user exists for the given request and if a project
     * has been loaded
     */
    isProjectLoaded: function(req, res, next) {
      if(!req.session.user) {
        return next(utils.error(401, "Unauthorized request"));
      }

      if(!req.session.project || !req.session.project.meta) {
        return next(utils.error(404, "No project information available for the user"));
      }
    ...
    /**
     * Login with publish.webmaker.org for the user
     */
    publishLogin: function(allowAnonymous) {
      return function(req, res, next) {
        var user = req.user;
        ...
        request({
          method: "POST",
          url: env.get("PUBLISH_HOSTNAME") + "/users/login",
          headers: {

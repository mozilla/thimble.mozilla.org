module.exports = function middlewareConstructor(env) {
  ...
  return {
    ...
    prepareMetaPage: function(nunjucksEnv, env) {
      ...
      return function(req, res, next) {
        var hostname = env.get("HOSTNAME"),
            ...
            detailsPage = nunjucksEnv.getTemplate('detailsPage.html'),
            detailsPageRendered = detailsPage.render({
              title: metaData.title,
              metaTags: metaTagsRender,
              hostname: hostname
            });

        req.body.detailsPage = detailsPageRendered;
        next();
      };
    },

    /**
     * Insert google analytics and the details
     * template into the sanitized details page.
     */
    finalizeProject: function(nunjucksEnv, env) {
      ...
      return function(req, res, next) {
        var hostname = env.get("HOSTNAME"),
            ...
            finalizedDetails = req.body.sanitizedDetails,

            // Details page
            detailsTemplate = nunjucksEnv.getTemplate('detailsTemplate.html'),
            detailsTemplateRendered = detailsTemplate.render({
              HOSTNAME: hostname,
              URL: escape( url ),
        ...
        finalizedDetails = finalizedDetails.replace(bodyRegExp, "<body$1>" + detailsTemplateRendered); 

        res.render("googleanalytics.html", function(err, results) {
          finalizedDetails = finalizedDetails.replace("</head", results + "</head");

          req.body.finalizedDetails = finalizedDetails;
          next();
        });
      };
    },
        ...
        s3.put(location + "_", headers)
          ...
          .on("response", function(res) {
            if (res.statusCode === 200) {
              var detailsPage = req.body.finalizedDetails;
              headers = {
                'x-amz-acl': 'public-read',
